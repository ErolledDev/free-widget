...
(function() {
  ...
  widget.style.zIndex = '999999'; // Ensure widget stays on top
  ...
  // Initial render
  widget.innerHTML = renderWidget(config);
  ...
  const updateInterval = setInterval(() => {
    ...
    if (currentConfigString !== lastConfig) {
      ...
      widget.innerHTML = renderWidget(currentConfig);
      
      // Re-animate after update
      setTimeout(() => {
        const widgetElement = widget.firstElementChild;
        if (widgetElement) {
          widgetElement.style.opacity = '1';
